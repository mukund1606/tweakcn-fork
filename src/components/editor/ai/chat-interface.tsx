...
export function ChatInterface() {
  ...
  const { checkValidSession } = useGuards();

  const hasMessages = messages.length > 0;
  const [editingMessageIndex, setEditingMessageIndex] = React.useState<number | null>(
    null,
  );

  const handleGenerateFromSuggestion = (promptData: AIPromptData | undefined) => {
    if (!checkValidSession("signup", "AI_GENERATE_FROM_CHAT_SUGGESTION", { promptData }))
      return;
